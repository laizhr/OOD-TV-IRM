...
for restart in range(flags.n_restarts):
    ...
    if flags.scheduler:
        ...
        # scheduler2 = CosineLR(optimizer2, flags.lr2, flags.steps)
    ...
    for step in tqdm(range(flags.steps)):
        ...
        for batch in range(train_batch_num):
            ...
            red = penalty_weight_flag * penalty_weight * train_penalty
            loss += red
    ...
    filePath = f"figure_data/{flags.dataset}_{algorithmMap[flags.irm_type]}.csv"
    df = pd.DataFrame(
        {
            "redRecord": redRecord,
