...
class NicoNet(nn.Module):
    def __init__(self, classNum):
        ...
        self.classNum1 = classNum
        self.resnet18.fc = torch.nn.Linear(self.num_ftrs, classNum)
...
class MultitaskModel(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(task_dims)):
            ...
            for j in range(1, len(task_dims[i])):
                if batch_norm:
                    curr_task_layers.append(
                        (f"Task{i + 1}BN{j}", nn.BatchNorm1d(task_dims[i][j - 1]))
                    )
    ...
    def forward(self, x):
        ...
        if isinstance(x, list):
            ...
            return [
                layer(intermed_out)
                for layer, intermed_out in zip(self.task_layers, intermed_outs)
            ]
        else:
...
class CNN1D(nn.Module):
    ...
    def __init__(self, output_size, in_channels, batch_norm=False, dropout_prob=0.0):
        ...
        self.feature_extractor = CNN1DFeatureExtractor(
            in_channels, output_size=128, batch_norm=False
        )
...
class CNN1DNoNegative(nn.Module):
    """
    CNN for time series classification
    """
...
class CNN1DMultitask(MultitaskModel):
    ...
    def __init__(
        self, in_channels, task_dims, batch_norm=False, use_idx=None, dropout_prob=0.0
    ):
        """
        Constructor.
        ...
        """
        feature_size = 128
        shared_layers = CNN1DFeatureExtractor(
            in_channels, output_size=feature_size, batch_norm=batch_norm
        )
        super().__init__(
            feature_size,
            task_dims,
