...
for restart in range(1):
    ...
    scale = torch.ones(9 if flags.subclass == "vehicle" else 10).cuda().requires_grad_()
    print("scale:",scale)
    ...
    for step in tqdm(range(flags.steps)):
        ...
        for batch in range(train_batch_num):
            ...
            train_nll, train_penalty, penalty_weight = algo(
                flags.norm, batch_data, step, mlp, mlp2, scale, mean_nll=mean_nll
            )
    ...
    bestResultPath = f"{BEST_PATH}/{flags.norm}_{flags.subclass}.csv"
