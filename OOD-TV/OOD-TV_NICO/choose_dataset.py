from utils_z import CELEBAZ_FEATURE, HousePrice, LOGITZ, LOGIT2Z, LANDCOVER, ADULT,MNIST,NICO
from utils import (
    LOGIT_LYDP,
...
from model import MLP2Layer, MLP2LayerNoNegative, MLP2LayerParameter
...
def init_landcover(flags):
    ...
    print(summary(mlp, input_size=( 8,46)))
...
def init_colored_mnist(flags):
    dp = MNIST(flags)
    mlp = dp.fetch_mlp().cuda()
    print(summary(mlp, input_size=( 3,28,28)))
    n_parameters = count_parameters(mlp)
    mlp2 = MLP2LayerParameter(flags, feature_dim=n_parameters, hidden_dim=32).cuda()
    print(summary(mlp2, input_size=(1, n_parameters)))
    test_batch_num = len(dp.test_loader)
