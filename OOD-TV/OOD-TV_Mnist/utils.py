...
def make_one_logit(num, sp_ratio, dim_inv, dim_spu):
    ...
    return {"images": inputs, "labels": outputs, "color": colors}
...
def make_logit_envs(total_num, flags):
    ...
    if flags.env_type == "linear":
        ...
        for i in range(envs_num):
            env_coef = lower_coef if i <= env_per_group else upper_coef
            envs.append(
                make_one_logit(num_per_env, env_coef, flags.dim_inv, flags.dim_spu)
            )
    else:
        raise Exception
...
def make_reg_envs(total_num, flags):
    ...
    envs.append(
        make_one_reg(
            total_num, sp_ratio_list[-1], inv_cond, flags.dim_inv, flags.dim_spu
        )
    )
    return envs
...
class CIFAR_LYPD(LYDataProvider):
    ...
    def test_batchs(self):
        return math.ceil(
            self.test_data.val_dataset.x_array.shape[0] / self.flags.batch_size
        )
...
class CosineLR(LambdaLR):
    ...
    def __init__(self, optimizer, lr, num_epochs, offset=1):
        self.init_lr = lr
        fn = (
            lambda epoch: lr * 0.5 * (1 + np.cos((epoch - offset) / num_epochs * np.pi))
        )
