...
class Infer_Irmv1_Multi_Class:
    ...
    def __init__(self, flags, dp):
        if flags.dataset == "logit_z":
            ...
            infer_env = InferEnvMultiClass(
                flags, z_dim=flags.aux_num, class_num=flags.z_class_num # 0 1
            ).cuda()
        else:
            raise Exception
    ...
    def __call__(
        self, batch_data, step, mlp=None, mlp2=None, scale=None, mean_nll=None, **kwargs
    ):
        ...
        if (
            ...
            for i in range(multi_loss.shape[1]):
                ...
                train_penalty = train_penalty + (grad1 * grad2).mean()
        else:
