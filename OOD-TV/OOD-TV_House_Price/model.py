...
class ENV_EBD(nn.Module):
    def __init__(self, flags):
        ...
        self.embedings = torch.nn.Embedding(flags.envs_num_train, 4)
        self.re_init()
...
class EBD(nn.Module):
    def __init__(self, envs_num_train):
        super(EBD, self).__init__()
        self.envs_num_train = envs_num_train
        self.embedings = torch.nn.Embedding(self.envs_num_train, 1)
        self.re_init()
    ...
    def re_init_with_noise(self, noise_sd):
        ...
        rd = torch.normal(
            torch.Tensor([1.0] * self.envs_num_train),
            torch.Tensor([noise_sd] * self.envs_num_train),
        )
...
class BayesW(nn.Module):
    ...
    def generate_rand(self, N):
        self.epsilon = list()
        for i in range(N):
            self.epsilon.append(torch.normal(torch.tensor(0.0), torch.tensor(1.0)))
    ...
    def variational_loss(self, xb, yb, N):
        ...
        for i in range(N):
            ...
            lk_loss += 1.0 / N * loss_i
        return lk_loss + 1.0 / self.flags.data_num * kl
...
class MLP2Layer(nn.Module):
    def __init__(self, flags, feature_dim, hidden_dim):
        super(MLP2Layer, self).__init__()
        self.flags = flags
        lin1 = nn.Linear(feature_dim, hidden_dim)
        lin2 = nn.Linear(hidden_dim, 1)
...
class MLP2LayerDoubleOutput(nn.Module):
    def __init__(self, flags, feature_dim, hidden_dim):
        super(MLP2Layer, self).__init__()
        self.flags = flags
        lin1 = nn.Linear(feature_dim, hidden_dim)
        ...
        for lin in [lin1, lin2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)
        self._main = nn.Sequential(lin1, nn.ReLU(True), lin2, nn.Softplus(True))
...
class MLP2LayerParameter(nn.Module):
    def __init__(self, flags, feature_dim, hidden_dim):
        super(MLP2LayerParameter, self).__init__()
        self.flags = flags
        lin1 = nn.Linear(feature_dim, hidden_dim)
        lin2 = nn.Linear(hidden_dim, 16)
        for lin in [lin1, lin2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)
        self._main = nn.Sequential(lin1, nn.ReLU(True), lin2, nn.Softplus(True),nn.Linear(16,1))
...
class PredEnvHatY(nn.Module):
    def __init__(self, flags):
        ...
        for lin in [self.lin1, self.lin2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)
        self._main = nn.Sequential(self.lin1, nn.ReLU(True), self.lin2)
...
class InferEnv(nn.Module):
    def __init__(self, flags, z_dim):
        ...
        for lin in [self.lin1, self.lin2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)
        self._main = nn.Sequential(self.lin1, nn.ReLU(True), self.lin2, nn.Sigmoid())
...
class PredEnvHatYSep(nn.Module):
    def __init__(self, flags):
        ...
        for lin in [self.lin1_1, self.lin1_2, self.lin2_1, self.lin2_2]:
            nn.init.xavier_uniform_(lin.weight)
            nn.init.zeros_(lin.bias)
        self._main1 = nn.Sequential(self.lin1_1, nn.ReLU(True), self.lin1_2)
        self._main2 = nn.Sequential(self.lin2_1, nn.ReLU(True), self.lin2_2)
    ...
    def forward(self, g, input):
        ...
        output[g == 0] = self._main1(input[g == 0].view(-1, 1)).view(-1)
        output[g == 1] = self._main2(input[g == 1].view(-1, 1)).view(-1)
        return output
...
class ResNet(nn.Module):
    ...
    def sep_param_id(self):
        sep_params = [
            ...
            if "classifier" in p[0] and "sep" in p[0]
        ]
    ...
    def rep_param_id(self):
        rep_param_id = [
            ...
            if id(p) not in self.sep_param_id() and id(p) not in self.share_param_id()
        ]
        return rep_param_id
    ...
    def get_optimizer_schedule(self, args):
        if args.irm_penalty_weight > 0:
            ...
            if args.lr_schedule_type == "step":
                ...
                scheduler_rep = lr_scheduler.StepLR(
                    optimizer_rep,
                    step_size=int(args.n_epochs / 2.5),
                    gamma=args.step_gamma,
                )
                scheduler_sep = lr_scheduler.StepLR(
                    optimizer_sep, step_size=int(args.n_epochs), gamma=args.step_gamma
                )
                scheduler_share = lr_scheduler.StepLR(
                    optimizer_share,
                    step_size=int(args.n_epochs / 2.5),
                    gamma=args.step_gamma,
                )

            return [optimizer_rep, optimizer_share, optimizer_sep], [
                scheduler_rep,
                scheduler_share,
                scheduler_sep,
            ]
            ...
            scheduler = lr_scheduler.StepLR(
                optimizer, step_size=int(args.n_epochs / 3.0), gamma=args.step_gamma
            )
