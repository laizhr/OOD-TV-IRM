...
class NICO(object):
    def __init__(self, flags):
        ...
        self.feature_dim = self.mlp.num_ftrs
        self.z_dim_ex = 3
    ...
    def fetch_train(self):
        ...
        # train_y = bs[]
        train_z = torch.mean(bs[0], dim=(2, 3))
        train_y = F.one_hot(bs[1], num_classes=self.classNum).float()
    ...
    def fetch_test(self):
        ...
        test_x = bs[0]  # 假设数据已经是张量格式
        # test_y = bs[]
        test_z = torch.mean(bs[0], dim=(2, 3))
        test_y = F.one_hot(bs[1], num_classes=self.classNum).float()
        return (
            test_x.cuda(),
            test_y.cuda(),
            test_z.cuda(),  # 移除lat_lon和climate
            None,
            # test_y.cuda(),
            # test_c.cuda(),
            None,
    ...
    def fetch_val(self):
        ...
        val_x = bs[0]  # 假设数据已经是张量格式
        # val_y = bs[]
        val_z = torch.mean(bs[0], dim=(2, 3))
        val_y = F.one_hot(bs[1], num_classes=self.classNum).float()
        return (
            val_x.cuda(),
            val_y.cuda(),
            # None,
