...
class MetaAcc(object):
    ...
    def process_batch(
        ...
        for e in range(self.env):
            ...
            batch_dict.update(
                {"acc_e%s" % e: env_acc, "data_num_e%s" % e: env_data_num}
            )
    ...
    def meta_acc(self):
        ...
        for e in range(self.env):
            ...
            acc_value = (
                1.0
                / meta_data_num["data_num_e%s" % e]
                * sum(
                    [x["acc_e%s" % e] * x["data_num_e%s" % e] for x in self.meta_list]
                )
            )
            full_dict.update({acc_env: acc_value})
            if self.acc_measure.__name__ == mean_accuracy_reg.__name__:
...
class ADULT(object):
    def __init__(self, flags):
        ...
        self.z_dim_ex = self.train_z.shape[1]
...
class CELEBAZ_FEATURE(object):
    ...
    def preprocess_data(self):
        ...
        # edited by w wang 2024-03-25
        train_x, train_y, train_z, train_g, train_c = process_file(train_file)
        test_file = f"datasets/CelebA/test_40000_0.999_0.8_20000_0.01_0.2_0.8_0.999.csv"
        # print(train_file)
        test_x, test_y, test_z, test_g, test_c = process_file(test_file)

        data_x = torch.cat((train_x, test_x))
        data_y = torch.cat((train_y, test_y))
        data_z = torch.cat((train_z, test_z))
        data_g = torch.cat((train_g, test_g))
        data_c = torch.cat((train_c, test_c))

        all_data_num = data_x.shape[0]
        rand_index = np.random.permutation(all_data_num)
        train_index = rand_index[:39996]
        test_index = rand_index[39996:]

        self.train_x, self.train_y = data_x[train_index, :], data_y[train_index, :]
        self.train_z, self.train_g = data_z[train_index, :], data_g[train_index, :]
        self.train_c = data_c[train_index, :]

        self.test_x, self.test_y = data_x[test_index, :], data_y[test_index, :]
        self.test_z, self.test_g = data_z[test_index, :], data_g[test_index, :]
        self.test_c = data_c[test_index, :]
...
class LANDCOVER(object):
    def __init__(self, flags):
        ...
        # create dataset
        train_dataset_ = init_dataset(config, "train")
        train_eval_dataset = init_dataset(config, "train")
        val_dataset_ = init_dataset(config, "val", train_dataset_)
        test_dataset = init_dataset(config, "test2", train_dataset_)
    ...
    def fetch_train(self):
        ...
        # added by w wang
        train_x = torch.squeeze(torch.reshape(train_x, (train_x.size(0), -1, 1)))
    ...
    def fetch_test(self):
        ...
        # added by w wang
        test_x = torch.squeeze(torch.reshape(test_x, (test_x.size(0), -1, 1)))
    ...
    def fetch_val(self):
        ...
        # added by w wang
        val_x = torch.squeeze(torch.reshape(val_x, (val_x.size(0), -1, 1)))
